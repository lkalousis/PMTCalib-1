...
Double_t _gausexpfunc( Double_t *x, Double_t *par )
{
  ...
  Double_t result = 0.0;
  if ( xx>=0.0 ) result = w*1.0*alpha*TMath::Exp( -xx*alpha ) + ( 1.0-w )/( sqrt( 2.0*TMath::Pi() )*s*gn )*TMath::Exp( -0.5*arg*arg );
...
Double_t _gammaexpfunc( Double_t *x, Double_t *par )
{
  ...
  Double_t result = 0.0;
  if ( xx>=0.0 ) result = w*1.0*alpha*TMath::Exp( -xx*alpha ) + ( 1.0-w )*lambda*(1.0+theta)*pow( lambda*(1.0+theta)*xx, theta )/TMath::Gamma(1.0+theta)*TMath::Exp( -lambda*(1.0+theta)*xx );
